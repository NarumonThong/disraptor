<template>
    <Layout>
        <div slot="buttons" class="form-group">
            <router-link class="btn" :to="{ name:'servicehistory-list' }">รายการข้อมูล</router-link>
            <router-link class="btn" :to="{ name:'servicehistory-form' }">เพิ่มข้อมูลใหม่</router-link>
        </div>
        <div class="card">
            <div class="card-body">
                <header class="mb-4">
                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        <tr v-for="item in servicehistorys" :key="item.RowKey">
...
export default {
  ...
  created() {
    this.$store.dispatch("set_servicehistorys");
  },
  methods: {
    onUpdate(item) {
      this.$router.push({ name: "servicehistorys-form", query: { id: item.m_id } });
    },
    // ลบข้อมูล
    onDelete(item) {
      this.alertify.confirm("คุณต้องการจะลบข้อมูลนี้จริงหรือ?", () => {
        Axios.delete(`/api/servicehistory/${item.m_id}`)
          .then(() => this.$store.dispatch("set_servicehistorys"))
